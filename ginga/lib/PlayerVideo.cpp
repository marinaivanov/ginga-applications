  ...
  if( !Player::getPrepared ())
    {
      ...
      string comando = "echo starting " + _id + "; date +\"%T.%N\"";
...
PlayerVideo::doPreparationActions ()
{
  ...
  if(percent == 100)
    {
      if (Player::getPrepared()==false){
        string comando = "echo preparadoooo " + _id + "; date +\"%T.%N\"";
        system (comando.c_str());
      }
...
void
PlayerVideo::enable_factory (gboolean enable) {
