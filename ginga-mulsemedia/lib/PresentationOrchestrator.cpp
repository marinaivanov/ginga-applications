...
void PreparationOrchestrator::printPreparationPlan ()
{
    ...
    g_print("Instant    Node        Action   \n");
    for( it = preparation_plan.begin(); it != preparation_plan.end(); it++ )
    {
       g_print("%.2fs - %s - %s \n", (*it).instant, (*it).vertex->getId().c_str(), (*it).action.c_str());
    } 
    g_print("---------------------------------------------------------------\n");*/
}
        ...
        for( it = preparation_plan.begin(); it != preparation_plan.end(); it++ )
        {
            ...
            if ((*it).instant > 0)
            {
                ...
                else if (strcmp((*it).vertex->getElementName().c_str(),"Effect")==0)
                {
                    ...
                    g_assert_nonnull(_effect);
                    _effect->addPreparationEvent("@lambda");
                    act.event = _effect->getPreparationEvent("@lambda");
                    
                    if (strcmp((*it).action.c_str(),"start")==0)
                    {
                        act.transition = Event::START;
                    }
                    else if (strcmp((*it).action.c_str(),"stop")==0)
                    {
                        act.event = _effect->getPresentationEvent("@lambda");
                        act.transition = Event::PREPARE_STOP;
                    }
                }
                ...
                if (_ctx != nullptr){
                    //Adiciona acoes para serem disparadas apos o inicio da apresentacao
                    //da midia que eh porta
